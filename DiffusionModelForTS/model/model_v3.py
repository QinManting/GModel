...
class NoisePredictorV3(nn.Module):
    ...
    def __init__(
        ...
        # 可学习的时间位置嵌入，便于模型显式识别 hour index
        self.time_pos_emb = nn.Embedding(seq_len, d_model)
        ...
        self.season_condition = SeasonConditionEncoding(5, d_model)
    ...
    def forward(self, x_t, t, season):
        ...
        # ========= 1. 保存跳跃连接的输入 =========
        # 为了与 interleaved tokens 保持一致，先把时间维移到第二维再展平
        # x_t: (B, C, L) -> (B, L, C) -> (B, L*C)
        x_t_flat = x_t.permute(0, 2, 1).reshape(B, self.seq_len * self.num_vars, 1)  # (B, N, 1)
        ...
        # ========= 5. 加时间位置编码（sinusoidal + 可学习） =========
        # 为每个时间步生成位置编码（sinusoidal）
        ...
        # 可学习的时间位置嵌入 (seq_len, d_model) -> (1,1,seq_len,d_model)
        time_ids = torch.arange(self.seq_len, device=device)
        time_learned = self.time_pos_emb(time_ids)[None, None, :, :]
        var_tokens = var_tokens + time_learned

        # ========= 6. interleaved temporal token =========
        # 当前 var_tokens: (B, num_vars, seq_len, d_model)
        # 先交换维度 -> (B, seq_len, num_vars, d_model)
        var_tokens = var_tokens.permute(0, 2, 1, 3)

        # 展平为 (B, seq_len * num_vars, d_model) 形成 [g1,p1,g2,p2,...]
        tokens = var_tokens.reshape(B, self.seq_len * self.num_vars, self.d_model)
        ...
        # (B, L*C, 1) -> (B, L, C)
        out = out.view(B, self.seq_len, self.num_vars)

        # -> (B, C, L)
        noises_pred = out.permute(0, 2, 1)

        return noises_pred


# 为了保持与v1接口兼容
NoisePredictor = NoisePredictorV3
